...
class PanduanInstalasiController extends GetxController with Apis {
  ...
  List<PanduanInstalasi> listData = [];
  RxList<PanduanInstalasi> data = <PanduanInstalasi>[].obs;
  int page = 1, total = 0;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.panduanInstalasi.getData(query);

      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PanduanInstalasi.fromJsonList(res.data);
      data.value = listData;
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future deleteData(int id) async {
    try {
      final res =
          await api.panduanInstalasi.deleteData(id).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  void insertData(PanduanInstalasi data) {
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.panduanInstalasi.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PanduanInstalasi.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(PanduanInstalasi data, int id) {
  ...
  Future onPaginate() async {
    try {
      ...
      final data = PanduanInstalasi.fromJsonList(res.data);
