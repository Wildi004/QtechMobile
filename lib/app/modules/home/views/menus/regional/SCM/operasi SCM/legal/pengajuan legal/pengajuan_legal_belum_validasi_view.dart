...
import 'package:qrm_dev/app/modules/home/views/menus/regional/SCM/operasi%20SCM/legal/pengajuan%20legal/edit_pengajuan_legal_view.dart';
...
class PengajuanLegalBelumValidasiView
    extends GetView<PengajuanLegalBelumValidasiController> {
  const PengajuanLegalBelumValidasiView({super.key});
              ...
              children: [
                ...data.generate((item, i) {
                  ...
                  return CustomScalaContainer(
                    child: Touch(
                        onTap: () {
                          Get.to(() => PengajuanLegalDetailView(data: item))
                              ?.then((value) {
                                ...
                                children: [
                                  IconButton(
                                    icon: const Icon(Hi.edit01,
                                        color: Colors.white),
                                    onPressed: () {
                                      Get.to(() => EditPengajuanLegalView(
                                          data: item))?.then((value) {
                                        if (value != null) {
                                          final form = Get.find<
                                              PengajuanLegalBelumValidasiController>();
                                          form.updateData(
                                              PengajuanLegal.fromJson(value),
                                              item.noHide!);
                                        }
                                      });
                                    },
