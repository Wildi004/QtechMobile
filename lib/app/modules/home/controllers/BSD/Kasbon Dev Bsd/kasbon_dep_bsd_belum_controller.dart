...
class KasbonDepBsdBelumController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.kasbonDepBsd.getDataBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.kasbonDepBsd.getDataBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = KasbonDepBsd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
