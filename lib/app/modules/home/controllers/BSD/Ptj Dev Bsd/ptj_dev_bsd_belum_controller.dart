...
class PtjDevBsdBelumController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.ptjDepBsd.getDataBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.ptjDepBsd.getDataBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PtjDepBsd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
