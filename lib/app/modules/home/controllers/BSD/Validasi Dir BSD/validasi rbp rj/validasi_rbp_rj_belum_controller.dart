...
class ValidasiRbpRjBelumController extends GetxController with Apis {
...
// material utama
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rbpRb.getDatarjbelum(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.rbpRb.getDatarjbelum({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = RbpRb.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.rbpRb.getDatarjbelum({...query, 'search': searchC.text});
