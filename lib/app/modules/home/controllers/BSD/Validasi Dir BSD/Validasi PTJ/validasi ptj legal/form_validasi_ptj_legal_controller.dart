...
class FormValidasiPtjLegalController extends GetxController with Apis {
  PtjLegal? data;
  ...
  PtjLegal details = PtjLegal();
  RxList<DetailPtjLegal> cards = RxList([]);
  ...
  Future getDetails(PtjLegal data) async {
    try {
      isLoading.value = true;
      String nohide = data.noHide ?? '';

      final res = await api.ptjGlobal.getDataByNoHidePtjLegal(nohide);

      details = PtjLegal.fromJson(res.data ?? {});
  ...
  List<DetailPtjLegal> dataPengajuan = [];
  ...
  Future<void> onSubmit(int? id) async {
    try {
      ...
      final res = await api.ptjGlobal
          .validasiLegal(payload, data!.noHide!)
          .ui
