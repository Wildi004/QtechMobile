...
class ValidasiRabBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rabGlobal.getAllRabBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.rabGlobal.getAllRabBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rabGlobal
          .getAllRabBelumVal({...query, 'search': searchC.text});
