...
class ArsipBsdController extends GetxController with Apis {
  ...
  List<ArsipDirBsd> listArsip = [];
  RxList<ArsipDirBsd> arsip = <ArsipDirBsd>[].obs;
  int page = 1, total = 0;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.arsipDirBsd.getData(query);

      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipDirBsd.fromJsonList(res.data);
  ...
  Future deleteData(int id) async {
    try {
      final res =
          await api.arsipDirBsd.deleteData(id).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  void insertData(ArsipDirBsd data) {
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.arsipDirBsd.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipDirBsd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.arsipDirBsd.getData({...query, 'search': searchC.text});
      final data = ArsipDirBsd.fromJsonList(res.data);
