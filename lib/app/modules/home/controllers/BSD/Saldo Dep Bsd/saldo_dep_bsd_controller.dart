...
class SaldoDepBsdController extends GetxController with Apis {
  ...
  List<SaldoDepBsd> listSaldo = [];
  RxList<SaldoDepBsd> saldo = <SaldoDepBsd>[].obs;
  ...
  Future getSaldoPtj() async {
    try {
      final res = await api.saldo.getDataSaldoPtjBsd();
  ...
  Future<void> getData() async {
    try {
      ...
      final res = await api.saldoDepBsd.getData(query);
      total = res.body?['pagination']?['total_records'] ?? 0;

      final dataList = SaldoDepBsd.fromJsonList(res.data);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.saldoDepBsd.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listSaldo = SaldoDepBsd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
      isLoading.value = false;
  ...
  void insertData(SaldoDepBsd data) {
    listSaldo.insert(0, data);
    // sortByTanggal(listSaldo);
    saldo.value = List.from(listSaldo);
  }

  void updateData(SaldoDepBsd data, int id) {
    try {
  ...
  Future<void> onPaginate() async {
    try {
      ...
      final res =
          await api.saldoDepBsd.getData({...query, 'search': searchC.text});
      final newData = SaldoDepBsd.fromJsonList(res.data);
