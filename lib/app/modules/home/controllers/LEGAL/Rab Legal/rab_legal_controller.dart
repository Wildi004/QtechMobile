...
class RabLegalController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rabGlobal.getDataValidasiLegal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.rabGlobal.getDataValidasiLegal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
  ...
  void insertData(RabLegal data) {
    try {
      final controller = Get.find<RabLegalBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rabGlobal
          .getDataValidasiLegal({...query, 'search': searchC.text});
