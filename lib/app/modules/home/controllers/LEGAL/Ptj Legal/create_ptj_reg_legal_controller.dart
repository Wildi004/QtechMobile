...
import 'package:qrm_dev/app/data/models/model%20legal/ptj_legal/detail_ptj.dart';

class CreatePtjRegLegalController extends GetxController with Apis {
  DetailPtjLegal? created;
  ...
  CreatePtjRegLegalController(this.type);
  final String type;

  RxInt grandTotal = 0.obs;

  List<PengajuanLegal> dataPengajuan = [];
  List<DetailPengajuanLegal> dataPengajuanDetails = [];
  List<DetailPtjLegal> dataPtj = [];
  ...
  Future<void> createPengajuan({required String type}) async {
    try {
      ...
      final res = await api.ptjGlobal.createPengajuanPtjLegal(type: type);
      forms.fill(res.data ?? {});
      created = DetailPtjLegal.fromJson(res.data ?? {});
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future<void> deletePtj(String noHide) async {
    try {
      await api.ptjGlobal.deletePtjLegal(noHide);
    } catch (e, s) {
      Errors.check(e, s);
    }
  }
  ...
  Future<void> getPengajuanDetails() async {
    if (dataPengajuanDetails.isEmpty) {
      final res = await api.pengajuanGlobal
          .getDataDetailLegal(limit: 'all')
          .ui
          .loading();
      dataPengajuanDetails = DetailPengajuanLegal.fromJsonList(res.data);
    }
  }
  ...
  Future<void> onSubmit() async {
    try {
      ...
      final res = await api.ptjGlobal
          .updateDataLegal(payload, created!.noHide!)
          .ui
          .loading('Memproses...');
