...
class CreatePengajuanLegalController extends GetxController with Apis {
  ...
  Future<void> createPengajuan() async {
    try {
      ...
      final res = await api.pengajuanGlobal.createPengajuanLegal();
      forms.fill(res.data ?? {});
      created = DetailPengajuanLegal.fromJson(res.data ?? {});
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future<void> getRab(String value, int index) async {
    if (value == 'RAB' && dataRabDetails.isEmpty) {
      final res =
          await api.rabGlobal.getDataRabLegalDetail(limit: 'all').ui.loading();
  ...
  Future<void> deletePengajuan(String noHide) async {
    try {
      await api.pengajuanGlobal.deleteDataLegal(noHide);
    } catch (e, s) {
      Errors.check(e, s);
    }
  }
  ...
  Future<void> onSubmit() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .updateDataPengajuanLegal(payload, created!.noHide!)
          .ui
          .loading('Memproses...');
