...
class PengajuanLegalBelumValidasiController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pengajuanGlobal.getDataLegalBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataLegalBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PengajuanLegal.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(PengajuanLegal data, String id) {
    try {
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataLegalBelumValidasi({...query, 'search': searchC.text});
