...
class KartuStokLogistikJktController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.kartuStok.getDataJkt(query);
  ...
  Future getDetail(String kodeStr) async {
    try {
      ...
      final res = await api.kartuStok.getDataDetailJkt(kodeStr);
      final body = res.body;
  ...
  Future deleteData(int id) async {
    try {
      final res =
          await api.kartuStok.deleteDataJkt(id).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.kartuStok.getDataJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = KartuStok.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.kartuStok.getDataJkt({...query, 'search': searchC.text});
