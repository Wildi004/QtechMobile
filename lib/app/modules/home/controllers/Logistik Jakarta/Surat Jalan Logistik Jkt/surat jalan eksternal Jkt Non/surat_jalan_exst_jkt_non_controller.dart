...
class SuratJalanExstJktNonController extends GetxController with Apis {
  ...
  RxList<SuratJalanExstNon> datapo = <SuratJalanExstNon>[].obs;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.suratJalanExstNon.getDataJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.suratJalanExstNon.getDataJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = SuratJalanExstNon.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void insertData(SuratJalanExstNon data) {
    try {
      final controller = Get.find<SuratJalanExstJktNonController>();
