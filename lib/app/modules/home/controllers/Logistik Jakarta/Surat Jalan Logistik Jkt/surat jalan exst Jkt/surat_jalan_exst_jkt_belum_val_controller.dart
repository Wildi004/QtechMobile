...
class SuratJalanExstJktBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.suratJalanExst.getBelumValJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.suratJalanExst.getBelumValJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = SuratJalanExst.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(SuratJalanExst data, String id) {
    try {
      final controller = Get.find<SuratJalanExstJktBelumValController>();
