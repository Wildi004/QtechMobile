...
class SuratJalanExtJktController extends GetxController with Apis {
  SuratJalanExst? data;
  ...
  RxList<SuratJalanExst> datapo = <SuratJalanExst>[].obs;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.suratJalanExst.getDataJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.suratJalanExst.getDataJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = SuratJalanExst.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void insertData(SuratJalanExst data) {
    try {
      final controller = Get.find<SuratJalanExtJktController>();
