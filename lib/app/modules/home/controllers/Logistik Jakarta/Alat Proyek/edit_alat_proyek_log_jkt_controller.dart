...
class EditAlatProyekLogJktController extends GetxController with Apis {
  ...
  XFile? fileImage;
  Rxn<AlatProyekLogJkt> aset = Rxn<AlatProyekLogJkt>();
  ...
  Future getDetailData(int id) async {
    try {
      Bindings.onRendered(() async {
        ...
        if (details == null) {
          final res = await api.alatProyekLogJkt.getDataDetail(id);
  ...
  Future getNamePm(int? id, {String? name}) async {
    ...
    final res = await api.dataProyek.getProyekBaratAll(limit: 'all');
    proyek = List<Map<String, dynamic>>.from(res.data ?? []);
