...
class InvDelPembJktNonPpnBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.invDelPembNonPpn.getBelumValidasiJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.invDelPembNonPpn
          .getBelumValidasiJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = InvDelPembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
