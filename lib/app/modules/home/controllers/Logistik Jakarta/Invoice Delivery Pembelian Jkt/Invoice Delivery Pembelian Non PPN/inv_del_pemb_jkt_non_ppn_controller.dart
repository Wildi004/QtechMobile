...
class InvDelPembJktNonPpnController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.invDelPembNonPpn.getDataJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.invDelPembNonPpn.getDataJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = InvDelPembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
