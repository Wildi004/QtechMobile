...
class DelPembPpnJktBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.delPembPpn.getBelumValJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.delPembPpn.getBelumValJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = DelPembPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(DelPembPpn data, String id) {
    try {
      final controller = Get.find<DelPembPpnJktBelumValController>();
  ...
  Future deleteData(String noHide) async {
    try {
      final res =
          await api.delPembPpn.deleteDataJkt(noHide).ui.loading('Menghapus...');
      if (!res.status) {
