...
class CreateDelPembJktNonPpnController extends GetxController with Apis {
  ...
  Future<void> createPemb() async {
    try {
      ...
      final res = await api.delPembNonPpn.createDataJkt();
      forms.fill(res.data ?? {});
  ...
  Future openPemb() async {
    ...
    try {
      if (pemb.isEmpty) {
        final res = await api.pembNonPpn.getDatajkt(query).ui.loading();
