...
class DelPembJktNonPpnBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.delPembNonPpn.getBelumValJkt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.delPembNonPpn.getBelumValJkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = DelPembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(DelPembNonPpn data, String id) {
    try {
