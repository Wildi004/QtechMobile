...
class EditPembPpnController extends GetxController with Apis {
  PembelianPpn? data;
  ...
  Future getDetails(PembelianPpn? data) async {
    try {
      ...
      cards.value = (details.detail ?? []).cast<DetailPembPpn>();
  ...
  Future getDetailData(String nohide) async {
    try {
      Bindings.onRendered(() async {
        ...
        if (details1 == null) {
          final res = await api.pembPpn.getDataNoHide(nohide);
          details1 = PembelianPpn.fromJson(res.data ?? {});
        }
        ...
        // ✅ Ambil data supplier dari API (jika mau isi dropdown options)
  ...
  void addPo() {
    ...
    // item baru => id null
    cards.insert(0, DetailPembPpn(id: null));
