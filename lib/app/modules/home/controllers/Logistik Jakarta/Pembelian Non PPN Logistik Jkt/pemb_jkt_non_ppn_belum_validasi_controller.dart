...
class PembJktNonPpnBelumValidasiController extends GetxController with Apis {
  PembNonPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pembNonPpn.getBelumValidasijkt(query);
  ...
  Future deleteData(String noHide) async {
    try {
      final res =
          await api.pembNonPpn.deleteDatajkt(noHide).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  void updateData(PembNonPpn data, String id) {
    try {
      final controller = Get.find<PembJktNonPpnBelumValidasiController>();
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.pembNonPpn.getBelumValidasijkt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
