...
import 'package:qrm_dev/app/data/models/models%20it/rab_validasi_it/rab_validasi_it.dart';

class CreatePengajuanItController extends GetxController with Apis {
  ...
  Future<void> createPengajuan() async {
    try {
      ...
      final res = await api.pengajuanGlobal.createPengajuanIt();
      forms.fill(res.data ?? {});
  ...
  Future<void> getRab(String value, int index) async {
    if (value == 'RAB' && dataRabDetails.isEmpty) {
      final res =
          await api.rabGlobal.getDataRabItDetail(limit: 'all').ui.loading();
  ...
  Future<void> deletePengajuan(String noHide) async {
    try {
      await api.pengajuanGlobal.deleteDataIt(noHide);
    } catch (e, s) {
      Errors.check(e, s);
    }
  }
  ...
  Future<void> onSubmit() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .updateDataPengajuanIt(payload, created!.noHide!)
          .ui
          .loading('Memproses...');
