...
class PengajuanItBelumValidasiController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pengajuanGlobal.getDataItBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataItBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PengajuanIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataItBelumValidasi({...query, 'search': searchC.text});
