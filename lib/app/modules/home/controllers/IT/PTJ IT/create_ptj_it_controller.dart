...
class CreatePtjItController extends GetxController with Apis {
  DetailPtjIt? created;
  ...
  CreatePtjItController(this.type);
  final String type;
  ...
  Future<void> createPengajuan({required String type}) async {
    try {
      ...
      final res = await api.ptjGlobal.createPengajuanPtjIt(type: type);
      forms.fill(res.data ?? {});
  ...
  Future<void> getPengajuanDetails() async {
    if (dataPengajuanDetails.isEmpty) {
      final res =
          await api.pengajuanGlobal.getDataItDetail(limit: 'all').ui.loading();
  ...
  Future<void> onSubmit() async {
    try {
      ...
      final res = await api.ptjGlobal
          .updateDataIt(payload, created!.noHide!)
          .ui
          .loading('Memproses...');
