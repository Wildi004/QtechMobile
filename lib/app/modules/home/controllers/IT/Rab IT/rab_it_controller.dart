...
class RabItController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rabGlobal.getDataValidasiIt(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.rabGlobal.getDataValidasiIt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
  ...
  void insertData(RabIt data) {
    try {
      final controller = Get.find<RabItBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rabGlobal
          .getDataValidasiIt({...query, 'search': searchC.text});
