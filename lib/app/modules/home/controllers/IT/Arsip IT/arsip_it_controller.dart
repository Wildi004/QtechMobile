...
class ArsipItController extends GetxController with Apis {
  ...
  List<ArsipIt> listArsip = [];
  RxList<ArsipIt> arsip = <ArsipIt>[].obs;
  int page = 1, total = 0;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.arsipIt.getData(query);

      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipIt.fromJsonList(res.data);
  ...
  Future deleteData(int id) async {
    try {
      final res = await api.arsipIt.deleteData(id).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.arsipIt.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.arsipIt.getData({...query, 'search': searchC.text});
      final data = ArsipIt.fromJsonList(res.data);
