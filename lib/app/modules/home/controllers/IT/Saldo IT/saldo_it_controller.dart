...
class SaldoItController extends GetxController with Apis {
  ...
  List<SaldoIt> listSaldo = [];
  RxList<SaldoIt> saldo = <SaldoIt>[].obs;
  ...
  Future getSaldoPtj() async {
    try {
      final res = await api.saldo.getSaldoIt();
  ...
  Future<void> getData() async {
    try {
      ...
      final res = await api.saldo.getDataSaldoIt(query);
      total = res.body?['pagination']?['total_records'] ?? 0;

      final dataList = SaldoIt.fromJsonList(res.data);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.saldo.getDataSaldoIt({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listSaldo = SaldoIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
      isLoading.value = false;
  ...
  void insertData(SaldoIt data) {
    listSaldo.insert(0, data);
    // sortByTanggal(listSaldo);
    saldo.value = List.from(listSaldo);
  }

  void updateData(SaldoIt data, int id) {
    try {
  ...
  Future<void> onPaginate() async {
    try {
      ...
      final res =
          await api.saldo.getDataSaldoIt({...query, 'search': searchC.text});
      final newData = SaldoIt.fromJsonList(res.data);
