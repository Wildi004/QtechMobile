...
import 'package:qrm_dev/app/data/models/models hrd/rab_validasi/rab_validasi.dart';
import 'package:qrm_dev/app/data/models/models%20hrd/rab_validasi/rab_detail.dart';

class RabHrdDetailController extends GetxController with Apis {
  RabValidasi? data;
  final isFilled = false.obs;
  RxBool isLoading = true.obs;

  // Grup per minggu + state expanded
  ...
  RabValidasi details = RabValidasi();
  RxList<RabValidasiDetail> cards = RxList<RabValidasiDetail>([]);
  RxList<FormManager> formDetails = RxList<FormManager>();

  Future getDetails(RabValidasi data) async {
    try {
      isLoading.value = true;

      final res = await api.rabGlobal.getDataRabHrdbyId(data.id!);
      details = RabValidasi.fromJson(res.data ?? {});
      ...
      final Map<int, List<RabValidasiDetail>> grouped = {};
