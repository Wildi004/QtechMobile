...
class RabHrdController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rabGlobal.getDataRabSudahValidasiHrd(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.rabGlobal
          .getDataRabSudahValidasiHrd({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
  ...
  void insertData(RabValidasi data) {
    try {
      final controller = Get.find<RabBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rabGlobal
          .getDataRabSudahValidasiHrd({...query, 'search': searchC.text});
