...
class ArsipKaryawanTimurController extends GetxController with Apis {
  ...
  Future getArsip() async {
    try {
      ...
      final res = await api.arsipKaryawanHrd.getDataTimur(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.arsipKaryawanHrd.getDataTimur({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listAK = ArsipKaryawanHrd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.arsipKaryawanHrd
          .getDataTimur({...query, 'search': searchC.text});
