...
class PtjHrdBelumValidasiController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.ptjGlobal.getDataBelumPtjHrd(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.ptjGlobal.getDataBelumPtjHrd({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPtj = PtjHrd.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.ptjGlobal
          .getDataBelumPtjHrd({...query, 'search': searchC.text});
