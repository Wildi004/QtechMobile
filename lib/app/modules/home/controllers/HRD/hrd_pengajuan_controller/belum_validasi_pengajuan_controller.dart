...
class BelumValidasiPengajuanController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pengajuanGlobal.getDataHrdBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataHrdBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PengajuanSudahValidasi.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(PengajuanSudahValidasi data, String id) {
    try {
      logg('test');

      int index = pengajuan.indexWhere((e) => e.noHide == id);
      logg('--- index: $index');

      if (index != -1) {
        pengajuan[index] = data;
        pengajuan.refresh();
      }
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataHrdBelumValidasi({...query, 'search': searchC.text});
