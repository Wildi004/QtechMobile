...
class PtjRndController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.ptjGlobal.getDataPtjRnd(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.ptjGlobal.getDataPtjRnd({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPtj = PtjIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void insertData(PtjIt data) {
    try {
      final controller = Get.find<PtjRndBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.ptjGlobal.getDataPtjRnd({...query, 'search': searchC.text});
