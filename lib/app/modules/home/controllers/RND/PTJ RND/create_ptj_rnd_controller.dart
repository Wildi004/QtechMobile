...
class CreatePtjRndController extends GetxController with Apis {
  DetailPtjIt? created;
  ...
  CreatePtjRndController(this.type);
  final String type;
  RxBool isSubmitted = false.obs;

  RxInt grandTotal = 0.obs;

  List<PengajuanIt> dataPengajuan = [];
  List<DetailPengajuanIt> dataPengajuanDetails = [];
  List<DetailPtjIt> dataPtj = [];
  ...
  Future<void> createPengajuan({required String type}) async {
    try {
      ...
      final res = await api.ptjGlobal.createPengajuanPtjRnd(type: type);
      forms.fill(res.data ?? {});
      created = DetailPtjIt.fromJson(res.data ?? {});
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future<void> getPengajuanDetails() async {
    if (dataPengajuanDetails.isEmpty) {
      final res =
          await api.pengajuanGlobal.getDataRndDetail(limit: 'all').ui.loading();
      dataPengajuanDetails = DetailPengajuanIt.fromJsonList(res.data);
    }
  }
  ...
  Future<void> deletePtj(String noHide) async {
    try {
      await api.ptjGlobal.deletePtjIt(noHide);
    } catch (e, s) {
      Errors.check(e, s);
    }
  }
  ...
  Future<void> onSubmit() async {
    try {
      ...
      logg('PAYLOAD: ${payload.toString()}', limit: 99999);
      final res = await api.ptjGlobal
          .updateDataRnd(payload, created!.noHide!)
          .ui
          .loading('Memproses...');
