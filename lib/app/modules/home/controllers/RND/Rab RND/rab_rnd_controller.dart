...
class RabRndController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rabGlobal.getDataValidasiRnd(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.rabGlobal.getDataValidasiRnd({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
  ...
  void insertData(RabIt data) {
    try {
      final controller = Get.find<RabRndBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rabGlobal
          .getDataValidasiRnd({...query, 'search': searchC.text});
