...
class PengajuanRndBelumController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pengajuanGlobal.getDataRndBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataRndBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PengajuanIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataRndBelumValidasi({...query, 'search': searchC.text});
