...
class CreatePengajuanRndController extends GetxController with Apis {
  ...
  Future<void> createPengajuan() async {
    try {
      ...
      final res = await api.pengajuanGlobal.createPengajuanRnd();
      forms.fill(res.data ?? {});
  ...
  Future<void> getRab(String value, int index) async {
    if (value == 'RAB' && dataRabDetails.isEmpty) {
      final res =
          await api.rabGlobal.getDataRabRndDetail(limit: 'all').ui.loading();
  ...
  Future<void> deletePengajuan(String noHide) async {
    try {
      await api.pengajuanGlobal.deleteDataRnd(noHide);
    } catch (e, s) {
      Errors.check(e, s);
    }
  }
