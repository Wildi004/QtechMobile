...
class PengajuanRndController extends GetxController with Apis {
  PengajuanIt? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pengajuanGlobal.getDataRndSudahValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataRndSudahValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listPengajuan = PengajuanIt.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void insertData(PengajuanIt data) {
    try {
      final controller = Get.find<PengajuanRndBelumController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pengajuanGlobal
          .getDataRndSudahValidasi({...query, 'search': searchC.text});
