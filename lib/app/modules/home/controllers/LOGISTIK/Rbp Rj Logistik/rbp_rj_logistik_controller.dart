...
class RbpRjLogistikController extends GetxController with Apis {
...
// material utama
  ...
  Future getData() async {
    try {
      ...
      final res = await api.rbpRb.getDataRj(query);
  ...
  // Future getDetailUser(String noHide) async {
  //   try {
  //     Bindings.onRendered(() async {
  //       Toast.overlay('Loading...', onCancel: () {});

  //       if (details == null) {
  //         final res = await api.rbpRbNohide.getDataDetail(noHide);

  //         details = RbpRbNohide.fromJson(res.data ?? {});
  //       }
  //       Toast.dismiss();
  //     });
  //   } catch (e, s) {
  ...
  Future<void> getDetailByNoHide(String noHide) async {
    try {
      ...
      final res = await api.rbpRbNohide.getDataDetailRj(noHide);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.rbpRb.getDataRj({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = RbpRb.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.rbpRb.getDataRj({...query, 'search': searchC.text});
