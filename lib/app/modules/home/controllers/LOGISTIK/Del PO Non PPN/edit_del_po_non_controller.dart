...
class EditDelPoNonController extends GetxController with Apis {
  DelPoNonPpn? data;
  ...
  Future getDetails(DelPoNonPpn? data) async {
    try {
      String? nohide = data?.noHide;
      final res = await api.delPoNonPpn.getDataNoHide(nohide);
      details = DelPoNonPpn.fromJson(res.data ?? {});
  ...
  Future getDetailData(String? nohide) async {
    try {
      Bindings.onRendered(() async {
        ...
        if (data == null) {
          final res = await api.delPoNonPpn.getDataNoHide(nohide);
          data = DelPoNonPpn.fromJson(res.data ?? {});
        }

        forms.fill(data!.toJson());

        await getPo(data?.noPo);
  ...
  Future<void> getPo(String? nohide) async {
    ...
    final option = pos.firstWhere(
      (e) => e['no_po_nonppn'].toString() == nohide.toString(),
      orElse: () => {},
    );
    ...
    if (option.isNotEmpty) {
      forms.set(
        'no_po_nonppn',
        Option(option['no_po_nonppn'], value: option['id']),
      );
    } else {}
  }
  ...
  Future<void> onSubmit() async {
    try {
      ...
      final payload = {
        ...
        'item_id': itemIds,
        "no_delivery": forms.get('no_delivery') ?? '',
