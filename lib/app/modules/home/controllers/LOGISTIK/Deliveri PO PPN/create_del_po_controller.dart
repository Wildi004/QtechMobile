...
class CreateDelPoController extends GetxController with Apis {
  DelPoPpn? created;
  ...
  Future<void> createPo() async {
    try {
      ...
      final res = await api.delPoPpn.createData();
      forms.fill(res.data ?? {});
      created = DelPoPpn.fromJson(res.data ?? {});
  ...
  Future openPo() async {
    ...
    try {
      if (po.isEmpty) {
        final res = await api.poPpn.getData(query).ui.loading();
        po = List<Map<String, dynamic>>.from(res.data ?? []);
        logg('[OPEN_PO] jumlah data PO: ${po.length}');
      }

      forms.set('no_po', '').options(
            po
                .where((e) => e['no_po'] != null && e['id'] != null)
                .map((e) => {
                      'label': e['no_po'].toString(),
                      'value': e['id'].toString(),
                    })
                .toList(),
          );
