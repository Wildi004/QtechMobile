...
class MonitorProyekTimurController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.monitorProyekBarat.getDataTimur(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.monitorProyekBarat
          .getDataTimur({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = MonitorProyekBarat.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
