...
class LapopranPoNonPpnController extends GetxController with Apis {
  RxInt tab = 0.obs;
  ...
  // 🔹 Hanya 2 filter: tanggal & bulan tahun
  final filter = [
    {'name': 'tanggal'},
    {'name': 'bulan tahun'},
  ];
  ...
  Future<void> fetchDeliveries() async {
    try {
      ...
      // 🔹 Ganti API jika berbeda untuk Non PPN
      final res = await api.poNon.getDataFilter(query);
      final body = res.body ?? {};
      ...
      if (body['status'] == true && body['data'] != null) {
        ...
        deliveryData.clear();
        Toast.warning('Tidak ada data PO Non PPN yang ditemukan');
      }
    } catch (e, s) {
      Errors.check(e, s);
    }
