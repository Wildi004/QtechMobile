...
class PoNonBelumValidasiController extends GetxController with Apis {
  PoNon? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.poNon.getDataBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.poNon.getDataBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PoNon.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
