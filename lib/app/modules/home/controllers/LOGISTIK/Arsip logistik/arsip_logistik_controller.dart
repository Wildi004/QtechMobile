...
class ArsipLogistikController extends GetxController with Apis {
  ...
  List<ArsipLogistik> listArsip = [];
  RxList<ArsipLogistik> arsip = <ArsipLogistik>[].obs;
  int page = 1, total = 0;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.arsipLogistik.getData(query);

      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipLogistik.fromJsonList(res.data);
  ...
  Future deleteData(int id) async {
    try {
      final res =
          await api.arsipLogistik.deleteData(id).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  void insertData(ArsipLogistik data) {
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.arsipLogistik.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listArsip = ArsipLogistik.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.arsipLogistik.getData({...query, 'search': searchC.text});
      final data = ArsipLogistik.fromJsonList(res.data);
