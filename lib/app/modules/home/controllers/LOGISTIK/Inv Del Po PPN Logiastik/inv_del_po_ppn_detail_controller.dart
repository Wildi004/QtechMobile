...
import 'package:qrm_dev/app/data/models/model%20logistik/inv_del_po_ppn/detail.dart';
import 'package:qrm_dev/app/data/models/model%20logistik/inv_del_po_ppn/inv_del_po_ppn.dart';

class InvDelPoPpnDetailController extends GetxController with Apis {
  InvDelPoPpn? data;
  ...
  InvDelPoPpn details = InvDelPoPpn();
  RxList<DetailInvDelPoPpn> cards = RxList([]);
  RxList<FormManager> formDetails = RxList<FormManager>();
  Future getDetails(InvDelPoPpn data) async {
    try {
      String nohide = data.noHide ?? '';
      logg('[DEBUG] Fetch detail dengan noHide: $nohide');

      final res = await api.invDelPoPpn.getDataNoHide(nohide);
      details = InvDelPoPpn.fromJson(res.data ?? {});
