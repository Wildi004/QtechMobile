...
class CreateInvDelPembPpnController extends GetxController with Apis {
  ...
  void onSelectPo(String id) async {
    ...
    final res = await api.delPembPpn.getDataNoHide(id);
    if (!res.status) {
  ...
  Future openInv() async {
    ...
    try {
      if (po.isEmpty) {
        final res = await api.delPembPpn.getData(query).ui.loading();
