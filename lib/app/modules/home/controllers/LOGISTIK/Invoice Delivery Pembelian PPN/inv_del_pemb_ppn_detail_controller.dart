...
import 'package:qrm_dev/app/data/models/model%20logistik/inv_del_pemb_ppn/detail.dart';
import 'package:qrm_dev/app/data/models/model%20logistik/inv_del_pemb_ppn/inv_del_pemb_ppn.dart';

class InvDelPembPpnDetailController extends GetxController with Apis {
  InvDelPembPpn? data;
  ...
  InvDelPembPpn details = InvDelPembPpn();
  RxList<DetailInvDelPembPpn> cards = RxList([]);
  RxList<FormManager> formDetails = RxList<FormManager>();
  Future getDetails(InvDelPembPpn data) async {
    try {
      String nohide = data.noHide ?? '';

      final res = await api.invDelPembPpn.getDataNoHide(nohide);
      details = InvDelPembPpn.fromJson(res.data ?? {});
