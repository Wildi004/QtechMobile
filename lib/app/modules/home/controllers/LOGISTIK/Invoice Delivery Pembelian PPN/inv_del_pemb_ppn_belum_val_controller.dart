...
class InvDelPembPpnBelumValController extends GetxController with Apis {
  InvDelPembPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.invDelPembPpn.getBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.invDelPembPpn.getBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = InvDelPembPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
