...
class PembNonPpnBelumValidasiController extends GetxController with Apis {
  PembNonPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pembNonPpn.getBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.pembNonPpn.getBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
