...
class PembNonPpnController extends GetxController with Apis {
  PembNonPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pembNonPpn.getData(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.pembNonPpn.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PembNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.pembNonPpn.getData({...query, 'search': searchC.text});
