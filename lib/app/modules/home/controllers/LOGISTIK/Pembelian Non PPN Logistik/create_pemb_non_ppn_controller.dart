...
class CreatePembNonPpnController extends GetxController with Apis {
  PembNonPpn? created;
  ...
  Future deleteData(String noHide) async {
    try {
      final res =
          await api.pembNonPpn.deleteData(noHide).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  Future<void> createPengajuan() async {
    try {
      ...
      final res = await api.pembNonPpn.createData();
      forms.fill(res.data ?? {});
  ...
  Future<void> onSubmit([int? id]) async {
    try {
      ...
      final payload = {
        ...forms.value,
        'no_invoice': forms.get('no_invoice') ?? '',
        // SUPPLIER
        'suplier_id': sup
            .firstWhere(
            ...
            ?.toString(), // ← backend minta STRING

        // ITEM
        'item_id': formDetail.map((f) => f.get('item_id')).toList(),

        // DETAIL BARANG
        'kode_material':
            formPo.map((e) => e.get('kode_material') ?? '').toList(),

        'nama_barang': formPo.map((e) => e.get('nama_barang') ?? '').toList(),

        'qty': formPo
            .map((e) => int.tryParse((e.get('qty') ?? '0').toString()) ?? 0)
            .toList(),

        'satuan_id': formPo
            .map((e) =>
                int.tryParse((e.extra('satuan_id') ?? '0').toString()) ?? 0)
            .toList(),

        'harga_satuan': formPo
            .map((e) =>
                int.tryParse((e.get('harga_satuan') ?? '0').toString()) ?? 0)
            .toList(),

        'diskon': formPo
            .map((e) => int.tryParse((e.get('diskon') ?? '0').toString()) ?? 0)
            .toList(),
      };
