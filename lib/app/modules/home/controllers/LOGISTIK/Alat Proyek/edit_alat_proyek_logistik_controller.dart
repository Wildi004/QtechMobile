...
class EditAlatProyekLogistikController extends GetxController with Apis {
  ...
  Future getDetailData(int id) async {
    try {
      Bindings.onRendered(() async {
        ...
        if (details == null) {
          final res = await api.alatProyek.getDataDetail(id);
          details = AlatProyek.fromJson(res.data ?? {});
        }
        ...
        await getReg(details?.regId);
        await getProyek(details?.proyekItemId);
        await getNamePm(details?.pm, name: details?.pmName);
  ...
  Future openProyek() async {
    try {
      if (proyek.isEmpty) {
        final res =
            await api.dataProyek.getProyekTimurAll(limit: 'all').ui.loading();
  ...
  Future getNamePm(int? id, {String? name}) async {
    if (id != null && name != null) {
      forms.set('pm', Option(name, value: id));
      return;
    }

    final res = await api.dataProyek.getProyekTimurAll(limit: 'all');
    proyek = List<Map<String, dynamic>>.from(res.data ?? []);

    for (var p in proyek) {
      final items = List<Map>.from(p['data_proyek_item'] ?? []);
      final found = items.firstWhere((e) => e['pm'] == id, orElse: () => {});
      if (found.isNotEmpty) {
        forms.set('pm', Option(found['pm_name'], value: found['pm']));
        break;
      }
    }
  }
  ...
  Future onSubmit([int? id]) async {
    try {
      ...
      if (form.ok) {
        ...
        if (id == null) {
          final res = await api.alatProyek
              .createData(payload)
              .ui
              .loading('Menambahkan...');
          if (res.status) {
