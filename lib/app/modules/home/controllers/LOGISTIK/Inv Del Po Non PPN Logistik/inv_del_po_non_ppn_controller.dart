...
class InvDelPoNonPpnController extends GetxController with Apis {
  InvDelPoNonPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.invDelPoNonPpn.getData(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.invDelPoNonPpn.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = InvDelPoNonPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  // void insertData(InvDelPoNonPpn data) {
  //   try {
  //     final controller = Get.find<PembPpnBelumValidasiController>();
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.invDelPoNonPpn.getData({...query, 'search': searchC.text});
