...
class DelPembPpnController extends GetxController with Apis {
  DelPembPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.delPembPpn.getData(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.delPembPpn.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = DelPembPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.delPembPpn.getData({...query, 'search': searchC.text});
