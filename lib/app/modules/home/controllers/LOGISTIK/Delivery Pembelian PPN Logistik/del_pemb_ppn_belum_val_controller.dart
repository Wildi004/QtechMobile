...
class DelPembPpnBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.delPembPpn.getBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.delPembPpn.getBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = DelPembPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future deleteData(String noHide) async {
    try {
      final res =
          await api.delPembPpn.deleteData(noHide).ui.loading('Menghapus...');
      if (!res.status) {
  ...
  Future onPaginate() async {
    try {
      ...
      final res =
          await api.delPembPpn.getBelumVal({...query, 'search': searchC.text});
