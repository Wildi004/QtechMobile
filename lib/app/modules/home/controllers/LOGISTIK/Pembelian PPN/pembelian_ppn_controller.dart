...
class PembelianPpnController extends GetxController with Apis {
  PembelianPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pembPpn.getData(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      page = 1;
      isLoading.value = true;
      final res = await api.pembPpn.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PembelianPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  Future onPaginate() async {
    try {
      ...
      final res = await api.pembPpn.getData({...query, 'search': searchC.text});
