...
class PembPpnBelumValidasiController extends GetxController with Apis {
  PembelianPpn? data;
  ...
  Future getData() async {
    try {
      ...
      final res = await api.pembPpn.getBelumValidasi(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.pembPpn.getBelumValidasi({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = PembelianPpn.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
