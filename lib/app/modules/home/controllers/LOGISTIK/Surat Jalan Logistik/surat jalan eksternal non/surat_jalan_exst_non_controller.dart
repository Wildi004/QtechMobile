...
class SuratJalanExstNonController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.suratJalanExstNon.getData(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.suratJalanExstNon.getData({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = SuratJalanExstNon.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void insertData(SuratJalanExstNon data) {
    try {
      final controller = Get.find<SuratJalanExstNonController>();
