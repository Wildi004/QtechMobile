...
class SuratJalanInternalBelumValController extends GetxController with Apis {
  ...
  Future getData() async {
    try {
      ...
      final res = await api.suratJalanInternal.getBelumVal(query);
  ...
  Future updateSearchQuery(String value) async {
    try {
      ...
      isLoading.value = true;
      final res =
          await api.suratJalanInternal.getBelumVal({...query, 'search': value});
      total = res.body?['pagination']?['total_records'] ?? 0;
      listData = SuratJalanInternal.fromJsonList(res.data);
    } catch (e, s) {
      Errors.check(e, s);
  ...
  void updateData(SuratJalanInternal data, String id) {
    try {
      final controller = Get.find<SuratJalanInternalBelumValController>();
