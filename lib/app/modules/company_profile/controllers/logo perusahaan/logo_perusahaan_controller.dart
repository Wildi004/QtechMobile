...
class LogoPerusahaanController extends GetxController with Apis {
  final forms = LzForm.make(['logo']);
  ...
  Future getData(int id) async {
    try {
      ...
      if (result.logo != null && result.logo!.isNotEmpty) {
        await imageC.loadImage(result.logo!);
      }
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future onSubmit() async {
    try {
      final form = forms.validate(required: ['logo']);
      if (!form.ok) return;
