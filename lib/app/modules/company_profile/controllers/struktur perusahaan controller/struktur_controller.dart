...
class StrukturController extends GetxController with Apis {
  final forms = LzForm.make(['struktur_organisasi']);
  ...
  Future getData(int id) async {
    try {
      ...
      if (result.strukturOrganisasi != null && result.strukturOrganisasi!.isNotEmpty) {
        await imageC.loadImage(result.strukturOrganisasi!);
      }
    } catch (e, s) {
      Errors.check(e, s);
    } finally {
  ...
  Future onSubmit() async {
    try {
      final form = forms.validate(required: ['struktur_organisasi']);
      if (!form.ok) return;

      final payload = form.value;
      if (file == null) {
        payload.remove('struktur_organisasi');
      } else {
        payload['struktur_organisasi'] = await api.toFile(file!.path);
      }
